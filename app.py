...
# 2. Sidebar - Gestión de Modo
st.sidebar.title("🛠️ Command Center")
st.sidebar.markdown("---")

# Interruptor Manual de Modo Demo
...
@st.cache_resource
def get_engine():
    if MOCK_MODE or not DATABASE_URL:
        return None
...
if run_pipeline:
        ...
        with st.spinner("Sintetizando Inteligencia Probabilística..."):
            if MOCK_MODE:
                ...
                history_df = pd.DataFrame({
                    'ds': [now - timedelta(days=i) for i in range(200, 0, -1)],
                    'y': [1000 + (i * 2) + (np.random.randn() * 50) for i in range(200)]
                })
                
                future_dates = [now + timedelta(days=i) for i in range(1, 366)]
                forecast_df = pd.DataFrame({
                    'ds': future_dates,
                    'yhat': [history_df['y'].iloc[-1] + (i * 1.5) for i in range(len(future_dates))],
                    'yhat_lower': [history_df['y'].iloc[-1] + (i * 1.5) - 100 - (i*0.5) for i in range(len(future_dates))],
                    'yhat_upper': [history_df['y'].iloc[-1] + (i * 1.5) + 100 + (i*0.5) for i in range(len(future_dates))]
                })
                ...
                report = DashboardReport(
                    ...
                    forecast_series=[], # Opcional para el grid
                    alerts=[
                        AuditInsight(
                            label="Seasonal Trend", value="Normal", level="info", 
                            description="Se detecta un incremento orgánico consistente con el crecimiento histórico."
                        ),
                        AuditInsight(
                            label="Argentine Holidays", value="Managed", level="info", 
                            description="Impacto de feriados nacionales mitigado por baja volatilidad."
                        )
                    ],
                    metadata=ReportMetadata(
                        model_version="Prophet_v1.1.5_MOCK",
            ...
            # Predicción
            fig.add_trace(go.Scatter(x=forecast_df['ds'], y=forecast_df['yhat'], mode='lines', line=dict(color='#58a6ff', width=3), name='Predicción Central'))
    ...
    st.info("Configura los parámetros y presiona 'Generar Inteligencia' arriba para comenzar.")
